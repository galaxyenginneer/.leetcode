...
class Solution {
    public int poorPigs(int buckets, int minutesToDie, int minutesToTest) {
        if (buckets<3) return Math.max(0,buckets-1);
        int num=minutesToTest/minutesToDie;
        // if (num==1) return buckets-2;
        double tmp;
        tmp=Math.log(buckets)/Math.log(num+1);     
        // int result=(num-1)*num+(int)Math.ceil(buckets/num);
        // result=Math.min(result,(num-1)*num+(int)Math.ceil(buckets/num));
