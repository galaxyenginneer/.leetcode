...
class Solution {
    boolean res=false;
    int [] s;
    public boolean search(int ind,int jmp){
        if (ind==s.length-1) {
            res=true;
            return true;
        }
        ...
        for (int i=s.length-1;i>ind;i--){
            if (Math.abs(s[i]-s[ind]-jmp)<=1) {
                if (search(i, s[i]-s[ind])) return true;
            }
            // if (s[i]-s[ind]-jmp>1) break;
        }
        return false;
    }
